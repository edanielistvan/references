...
/// </summary>
gCamera::gCamera(void) : m_eye(0.0f, 20.0f, 20.0f), m_at(0.0f), m_up(0.0f, 1.0f, 0.0f), m_speed(6.0f), m_goFw(0), m_goRight(0), m_goUp(0), m_slow(true)
{
...
gCamera::gCamera(glm::vec3 _eye, glm::vec3 _at, glm::vec3 _up) : m_speed(6.0f), m_goFw(0), m_goRight(0), m_goUp(0), m_dist(10), m_slow(true)
{
...
void gCamera::KeyboardUp(SDL_KeyboardEvent& key)
{
	...
	if (!m_slow && m_goFw == 0 && m_goRight == 0 && m_goUp == 0) {
		m_slow = true;
		m_speed /= 4.0f;
	}
}
...
void gCamera::SetEye(glm::vec3 _eye)
{
	SetView(_eye, m_at, m_up);
}
